"""Typer and Rich CLI interface for DPX-Cairo."""
...
app = typer.Typer(
    name="dpx-cairo",
    help="⚡ Enterprise Cairo & Starknet Static Analyzer: Components, Storage Mapping, Account Abstraction, Syscalls, Upgrades, Events & GoF 23.",
...
def scan(
    path: Annotated[
        str,
        typer.Argument(
            help="Path to a Cairo source file (.cairo) or directory containing a Scarb package (Scarb.toml).",
        ),
    ] = ".",
    ...
    pattern: Annotated[
        ...
        typer.Option(
            "--pattern",
            "-p",
            help="Filter for specific pattern type(s) (e.g. 'starknet_contract_component_composition', 'upgradeable_contract_class_hash').",
        ),
    ] = None,
    ...
    exclude: Annotated[
        ...
        typer.Option(
            "--exclude",
            "-e",
            help="Directory name(s) to exclude from scanning (e.g. -e target -e .starknet_artifacts).",
        ),
    ] = None,
    verbose: Annotated[
        bool,
    ...
    """Scan a Cairo / Starknet smart contract package for components, storage layouts, syscalls, upgradeable proxies, GoF patterns, and security hazards."""
...
@app.command()
def rules() -> None:
    """List all available Cairo detection rules and quality checks."""
    table = Table(
        title="⚡ DPX-Cairo: Supported Pattern Rules & Quality Checks",
...
def info(
    pattern_name: Annotated[
        str,
        typer.Argument(
            help="Pattern type or name to query (e.g. 'starknet_contract_component_composition', 'upgradeable_contract_class_hash').",
        ),
    ],
) -> None:
    """Display deep architectural details, rationale, and advice for a specific Cairo pattern."""
    ...
    panel = Panel.fit(
        f"[bold white]Pattern:[/bold white] [bold red]{pdef.name}[/bold red]\n"
        ...
        f"[bold white]Cairo Target:[/bold white] {pdef.cairo_version}\n\n"
        f"[bold white]Description:[/bold white]\n{pdef.description}\n\n"
        f"[bold white]Architecture Recommendation:[/bold white]\n{pdef.recommendation or 'Follow standard Cairo & Starknet best practices.'}",
        title=f"🔍 Cairo Architecture Encyclopedia: {pdef.name}",
        border_style="red",
    )
    console.print(panel)
