"""Interactive HTML Observability HUD Formatter for Cairo & Starknet."""
...
CATEGORY_COLORS: dict[PatternCategory, dict[str, str]] = {
    PatternCategory.CAIRO_IDIOMATIC_STARKNET: {
        "text": "#ff385c",
        "bg": "#ff385c22",
        "border": "#ff385c",
        "accent": "#ff6382",
        "name": "Starknet & Sierra Idiomatic",
    },
    PatternCategory.COMPONENTS_UPGRADES: {
        "text": "#ff8533",
        "bg": "#ff853322",
        "border": "#ff8533",
        "accent": "#ffa366",
        "name": "Components & Upgradability",
    },
    PatternCategory.EVENTS_ERRORS_ZK: {
        "text": "#50fa7b",
        "bg": "#50fa7b22",
        "border": "#50fa7b",
        "accent": "#73fa96",
        "name": "Events, Errors & ZK Hashes",
    },
    ...
    PatternCategory.STARKNET_SECURITY_HAZARDS: {
        "text": "#ff5555",
        "bg": "#ff555522",
        "border": "#ff5555",
        "accent": "#ff6e6e",
...
class HtmlReportFormatter(ReportFormatterPort):
    """Renders standalone, responsive HTML dashboard for Cairo DetectionReport with AI Prompt Generator."""
    ...
    def format(self, report: DetectionReport, verbose: bool = False) -> str:
        ...
        for idx, det in enumerate(report.detections, 1):
            ...
            finding_prompt = (
                f"Refactor and optimize the following Cairo / Starknet smart contract architectural finding:\n"
                ...
                f"Please provide an idiomatic Cairo 1.0 - 2.8+ / Starknet implementation adhering to modular Component composition, storage mapping best practices, caller authorization checks, and gas-efficient execution steps."
            )
...
<head>
    ...
    <title>Pattern Scanner Report - DPX-Cairo Architecture HUD - {html.escape(report.project_path or "Codebase")}</title>
    <style>
        :root {{
            --bg: #0b0f19;
            --card-bg: #141b2d;
        ...
        * {{ box-sizing: border-box; margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, monospace, sans-serif; }}
        ...
        .btn-ai-context {{
            background: linear-gradient(135deg, #ff385c 0%, #ff8533 100%);
            color: #ffffff;
            border: none;
            padding: 10px 18px;
            border-radius: 8px;
        ...
        .btn-view-prompt:hover {{
            border-color: var(--cairo-crimson);
            color: var(--cairo-crimson);
        }}
        ...
        .search-bar:focus {{ border-color: var(--cairo-crimson); }}
        ...
        .pattern-card:hover {{ border-color: #ff385c88; box-shadow: 0 4px 20px rgba(0,0,0,0.3); }}
        .card-header {{ background: #1a233a; padding: 12px 18px; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 10px; border-bottom: 1px solid var(--border); }}
        ...
        code {{ background: #0b0f19; padding: 3px 6px; border-radius: 4px; font-family: ui-monospace, SFMono-Regular, Menlo, monospace; color: #ff385c; border: 1px solid #232f48; }}
        ...
        .location-tag {{ color: #ff385c; font-size: 11px; margin-left: 6px; }}
...
<body>
    <div class="container">
        <header>
            <div>
                <h1>⚡ DPX-Cairo Architecture & Starknet Observability HUD</h1>
                <div class="subtitle">Components, Storage Mapping, Account Abstraction, Syscalls, Upgrades, Events & GoF 23 • Target: <code>{html.escape(report.project_path or "Target Repository")}</code></div>
            </div>
        ...
        <input type="text" id="searchInput" class="search-bar" placeholder="🔎 Instant search by pattern name, category, target contract/function, or component (e.g. component, upgrade, storage, syscall, l1_handler)...">
    ...
    <div id="promptModal" class="modal-overlay" onclick="closeModalOnOverlay(event)">
        <div class="modal-box">
            <div class="modal-header">
                <div class="modal-title">🤖 AI Smart Contract Architect Context Prompt (Cairo & Starknet)</div>
                <button class="modal-close" onclick="closePromptModal()">&times;</button>
            </div>
            <div class="modal-body">
                <p style="font-size: 13px; color: #94a3b8; margin-bottom: 12px;">Structured XML/Markdown context for LLMs (Claude, GPT-4, Gemini) detailing Starknet components, storage mapping, account abstraction, syscalls, upgradeable proxies, events, and security hazards.</p>
    ...
    <script>
        ...
        filterBtns.forEach(btn => {{
            btn.addEventListener('click', () => {{
                ...
                btn.style.outline = '2px solid #ff385c';
                filterCards();
            }});
        }});
    </script>
