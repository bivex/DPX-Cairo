"""Token-Efficient LLM / AI Prompt Context Formatter for Cairo & Starknet."""
...
class LlmReportFormatter:
    ...
    def format_scan_report(self, report: DetectionReport) -> str:
        ...
        lines: list[str] = [
            '<codebase_architecture_analysis language="cairo">',
        ...
        patterns = [d for d in report.detections if d.pattern_category not in (PatternCategory.PRINCIPLE, PatternCategory.STARKNET_SECURITY_HAZARDS)]
        hazards = [d for d in report.detections if d.pattern_category in (PatternCategory.PRINCIPLE, PatternCategory.STARKNET_SECURITY_HAZARDS)]

        if patterns:
            lines.append("    <cairo_starknet_components_and_patterns>")
            for d in patterns:
        ...
        if hazards:
            lines.append("    <starknet_security_hazards>")
            for v in hazards:
