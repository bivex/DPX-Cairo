"""Console Rich Formatter for Cairo & Starknet pattern detection findings."""
...
class ConsoleReportFormatter(ReportFormatterPort):
    ...
    def format(self, report: DetectionReport, verbose: bool = False) -> str:
        ...
        # Header Banner
        header = Panel.fit(
            f"[bold white]⚡ DPX-Cairo: Components, Storage Mapping, Syscalls, Account Abstraction & GoF 23 (Cairo 1-2+ / Starknet)[/bold white]\n"
        ...
        # Summary Table
        table = Table(title="📊 Detection Summary by Category", title_style="bold red", border_style="dim")
        table.add_column("Pattern Category", style="bold white")
        table.add_column("Detections", justify="right", style="bold red")
        ...
        console.print(table)
        console.print("\n[bold]📋 Identified Starknet Smart Contract Patterns & Security Signals:[/bold]\n")
        ...
        for idx, det in enumerate(report.detections, 1):
            ...
            tree = Tree(f"[bold white]#{idx} {det.pattern_type.value.upper()}[/bold white] on [red]{det.target_kind}[/red] '[bold white]{det.target_name}[/bold white]'")
            if det.primary_location:
                tree.add(f"📍 [dim]Location:[/dim] [red]{det.primary_location}[/red]")
            ...
            if det.evidences:
                ...
                for ev in det.evidences:
                    ...
                    loc_str = f" → [red]{ev.location}[/red]" if ev.location else ""
