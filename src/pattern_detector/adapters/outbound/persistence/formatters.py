"""JSON, Markdown, SARIF formatters & persistence repositories for Cairo & Starknet."""
...
class MarkdownReportFormatter(ReportFormatterPort):
    ...
    def format(self, report: DetectionReport, verbose: bool = False) -> str:
        lines = [
            f"# ⚡ DPX-Cairo: Cairo & Starknet Smart Contract Architectural Pattern Report",
            f"",
...
class SarifReportFormatter(ReportFormatterPort):
    ...
    def format(self, report: DetectionReport, verbose: bool = False) -> str:
        ...
        for det in report.detections:
            ...
            level = "error" if det.pattern_category.value in ("starknet_security_hazards", "principle") else "note"
            ...
            "runs": [
                {
                    "tool": {
                        "driver": {
                            "name": "DPX-Cairo",
                            "informationUri": "https://github.com/bivex/DPX-Cairo",
                            "version": "0.1.0",
                        }
                    },
                    "results": results,
