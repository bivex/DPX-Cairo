"""Inbound driving ports for DPX-Cairo."""
...
@dataclass
class ScanOptions:
    """Configuration options for a Cairo scanning session."""

    min_confidence: float = 0.0
    enabled_patterns: list[str] = field(default_factory=list)
    file_extensions: list[str] = field(default_factory=lambda: [".cairo"])
...
class ScannerPort(Protocol):
    """Inbound port for scanning a target directory or Cairo source file."""
